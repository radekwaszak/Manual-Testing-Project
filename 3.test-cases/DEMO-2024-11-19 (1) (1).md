...
# **TC-5 Verify login fails with incorrect password**
...
# **TC-6 Verify search functionality with valid product name (non-signed- in user)**
...
# **TC-7 Verify adding a single product to the cart (non-signed-in user)**
...
# **TC-8 Verify adding products to the cart in different variants and quantities (non-signed-in user)**
...
# **TC-9 Verify product quantity change from the cart level (non-signed- in user)**
